...
object ColouredMetaballs extends IndigoShader:
  ...
  val config: GameConfig =
    ColouredMetaballsConfig.config.noResize

  val assets: Set[AssetType]      = ColouredMetaballsAssets.assets.assetSet
    ...
    Shader[FragmentEnv] { env =>
      ...
      def fragment(color: vec4): vec4 =
        ...
        _for(1.0f, _ < count, _ + 1.0f) { i =>
          ...
          accColor = accColor + ((c * influence) / count)
          accAlpha = clamp(accAlpha + (influence / count), 0.0f, 1.0f)
        }

        val col = vec4(accColor * accAlpha, accAlpha)
        smoothstep(col, vec4(0.0f), vec4(0.05f))
